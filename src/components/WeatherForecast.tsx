...
import { type WeatherCondition, wmoCodeToCondition } from "../utils/weather";
    ...
    return (
      <div className="card-technical rounded p-6 animate-fade-in">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm font-mono-data text-teal-400/60">{t("weather.loading")}</span>
        </div>
      </div>
    );
  }
  ...
  if (error || !weather) {
    return (
      <div className="card-technical rounded p-6 animate-fade-in border-l-4 border-l-orange-500/50">
        <div className="text-sm font-mono-data text-orange-400/80">{t("weather.loadError")}</div>
      </div>
    );
  }
      ...
      <div className="overflow-x-auto -mx-2 px-2">
        <div className="grid grid-cols-7 gap-2 min-w-[560px]">
          {weather.days.map((day) => {
            ...
            return (
              <div key={day.date} className="data-panel p-3 rounded text-center space-y-2">
                {/* Day name */}
                ...
                {/* Temperature */}
                <div>
                  <div className="text-sm font-mono-data glow-amber">{day.tempMax}°</div>
                  <div className="text-xs font-mono-data text-gray-500">{day.tempMin}°</div>
                </div>
