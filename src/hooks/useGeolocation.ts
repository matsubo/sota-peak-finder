...
import {
  findJccJcgByCity,
  findLocationInfo,
  findNearbySotaSummits,
  getElevation,
  reverseGeocode,
} from "../utils/api";
import { calculateGridLocator, convertToDMS } from "../utils/coordinate";
  ...
  const fetchLocation = useCallback(async () => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        findNearbySotaSummits(lat, lon, sotaData, 20, 5000)
          .then((summits) => {
          ...
          initialData.sotaSummits.forEach((summit) => {
            if (currentElevation !== null) {
              ...
              summit.isActivationZone =
                verticalDistance <= 25 && verticalDistance >= -25 && summit.distance <= 100;
            } else {
