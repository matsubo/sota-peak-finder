import { MapPin, Flag as Mountain, Navigation, RefreshCw } from "lucide-react";
...
import type { SotaSummitWithDistance } from "../types/location";
...
export function NearbyPage() {
  ...
  const [overrideSummits, setOverrideSummits] = useState<SotaSummitWithDistance[] | null>(null);
  ...
  useEffect(() => {
    if (location?.sotaSummits && location.sotaSummits.length > 0) {
      const nearestSummit = location.sotaSummits[0];
      trackSotaSummitView(location.sotaSummits.length, nearestSummit.ref, nearestSummit.distance);
    }
  }, [location]);
    ...
    <div className="min-h-screen p-3 sm:p-4 md:p-5 relative z-10">
      <div className="mx-auto max-w-6xl">
        ...
        <main className="space-y-4">
          ...
          <div className="radio-panel rounded-sm p-3 animate-fade-in">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div className="font-mono-data text-xs tracking-widest text-teal-300">
                  {t(status)}
                </div>
              </div>
            ...
            <div className="card-technical rounded-none animate-fade-in overflow-hidden corner-accent">
              ...
              <div className="bg-black/20">
                <div className="px-5 py-3 border-l-4 border-l-green-500">
                  <div className="text-[10px] font-mono-data glow-green tracking-wider mb-2">
                    {t("nearbyPage.gpsCoordinates")}
                  </div>
          ...
          {location &&
            (overrideSummits || location.sotaSummits) &&
            ((overrideSummits || location.sotaSummits)?.length ?? 0) > 0 && (
              <div className="animate-fade-in space-y-3">
                  ...
                  <div key={summit.ref} className="flex items-stretch">
                    ...
                    >
                      ...
                      <div className="col-span-3 flex flex-col justify-center">
                        ...
                        <div className="text-teal-100/80 text-xs truncate">{summit.name}</div>
                      </div>
                      ...
                      <div className="col-span-2 flex items-center gap-1.5">
                        <div className="w-5 h-5 rounded-full border border-teal-500/40 flex items-center justify-center bg-black/40">
                          <Navigation
                            ...
                            style={{ transform: `rotate(${summit.bearing}deg)` }}
                          />
                        </div>
                        <span className="font-mono-data text-teal-300 text-xs">
...
function ResultItem({ label, value, highlight }: ResultItemProps) {
  ...
  if (highlight) {
    return (
      <div className="data-panel rounded p-3 text-center relative">
        ...
        <div className="font-mono-data text-2xl glow-amber tracking-wider">{displayValue}</div>
  ...
  return (
    <div className="py-0.5">
      ...
      <div className="font-mono text-base text-teal-100 tracking-wide">{displayValue}</div>
    </div>
  );
}
